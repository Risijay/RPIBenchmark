...
echo -e "Current Temp:" vcgencmd measure_temp

echo -e "Running CPU test...\e[94m"
sysbench --num-threads=4 --validate=on --test=cpu --cpu-max-prime=6000 run | grep 'total time:\|min:\|avg:\|max:' | tr -s [:space:]
vcgencmd measure_temp
echo -e "\e[93m"
...
sysbench --num-threads=4 --validate=on --test=memory --memory-block-size=1K --memory-total-size=5G --memory-access-mode=seq run | grep 'Operations\|transferred\|total time:\|min:\|avg:\|max:' | tr -s [:space:]
vcgencmd measure_temp
echo -e "\e[93m"
...
echo -e "Final Temp:" vcgencmd measure_temp
